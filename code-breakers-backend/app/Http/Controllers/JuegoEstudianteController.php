   ...
   public function getAvailableActivities(Request $request)
{
    try {
        ...
        $actividades = $query->orderBy('created_at', 'desc')->paginate(12);

        // Normalizar campos por consistencia
        $data = $actividades->getCollection()->transform(function ($actividad) {
            return [
                'id' => $actividad->id,
                '_id' => $actividad->id, // Para compatibilidad
                'title' => $actividad->title ?: $actividad->nombre,
                'nombre' => $actividad->title ?: $actividad->nombre,
                'description' => $actividad->description ?: $actividad->descripcion ?: 'Sin descripción',
                'descripcion' => $actividad->description ?: $actividad->descripcion ?: 'Sin descripción',
